...
func (sp *SessionPool) refreshSessions() {
	started := time.Now()
	glog.V(common.DEBUG).Info("Starting session refresh manifestID=", sp.mid)
	defer glog.V(common.DEBUG).Infof("Ending session refresh manifestID=%s dur=%s orchs=%d", sp.mid, time.Since(started),
		sp.sel.Size())
	defer func() {
	...
	glog.Infof("====> sess map %d uniq %d", len(sp.sessMap), len(uniqueSessions))
...
func (sp *SessionPool) selectSessions(sessionsNum int) []*BroadcastSession {
	...
	glog.Infof("---> pool size %d sessmap %d sel size %d", sp.poolSize, len(sp.sessMap), sp.sel.Size())
	...
	checkSessions := func(m *SessionPool) bool {
		...
		res := (numSess > 0 || len(sp.lastSess) > 0)
		glog.Infof("===$$ check sessions res %v numSess %d lastSess %d", res, numSess, len(sp.lastSess))
	...
	for checkSessions(sp) {
		...
		if sess == nil {
			glog.Infof("******************** no new sessions are available")
			glog.Infof("******************** no new sessions are available")
			glog.Infof("******************** no new sessions are available")
...
type BroadcastSessionsManager2 struct {
	mid              core.ManifestID
...
func NewSessionManager2(node *core.LivepeerNode, params *core.StreamParameters, sel BroadcastSessionsSelectorFactory) *BroadcastSessionsManager2 {
	var trustedPoolSize, untrustedPoolSize float64
	if node.OrchestratorPool != nil {
		trustedPoolSize = float64(node.OrchestratorPool.SizeWithScore(common.Score_Trusted))
		untrustedPoolSize = float64(node.OrchestratorPool.SizeWithScore(common.Score_Untrusted))
	}
	...
	createSessionsTrusted := func() ([]*BroadcastSession, error) {
		return selectOrchestrator(node, params, trustedNumOrchs, susTrusted, common.Score_Trusted)
	}
	createSessionsUntrusted := func() ([]*BroadcastSession, error) {
		return selectOrchestrator(node, params, untrustedNumOrchs, susUntrusted, common.Score_Untrusted)
	}
	glog.Infof("~~~~~~~ trusted pool size %v unpolsize %v tnumo %d untnumo %d", trustedPoolSize, untrustedPoolSize, trustedNumOrchs, untrustedNumOrchs)
	bsm := &BroadcastSessionsManager2{
...
func (bsm *BroadcastSessionsManager2) suspendAndRemoveOrch(sess *BroadcastSession) {
...
func (bsm *BroadcastSessionsManager2) removeSession(session *BroadcastSession) {
	bsm.sessLock.Lock()
...
func (bsm *BroadcastSessionsManager2) pushSegInFlight(sess *BroadcastSession, seg *stream.HLSSegment) {
	bsm.sessLock.Lock()
	defer bsm.sessLock.Unlock()
...
func (bsm *BroadcastSessionsManager2) selectSessions() ([]*BroadcastSession, bool) {
	bsm.sessLock.Lock()
	defer bsm.sessLock.Unlock()
	...
	if bsm.VerificationFreq > 0 {
		...
		glog.Infof("===> trusted sessions %d untrusted %d calc %v", len(sessions), len(untrustedSesions), calcPerceptualHash)
...
func (bsm *BroadcastSessionsManager2) cleanup() {
	bsm.sessLock.Lock()
	defer bsm.sessLock.Unlock()
	bsm.finished = true
...
// func (bsm *BroadcastSessionsManager2) chooseResults(submitResults []*SubmitResult) (*BroadcastSession, *ReceivedTranscodeResult, error)
func (bsm *BroadcastSessionsManager2) chooseResults(submitResultsCh chan *SubmitResult,
	...
	glog.Infof("################################################################### got trusted segments:")
	for _, seg := range trustedResults.TranscodeResult.Segments {
		glog.Infof("### url=%s pixels=%d hash=%s", seg.Url, seg.Pixels, seg.PerceptualHashUrl)
	}
	if len(untrustedResults) == 0 {
	...
	for _, untrustedResult := range untrustedResults {
		...
		if err != nil {
			err = fmt.Errorf("error comparing perceptual hashes from url=%s err=%w",
				untrustedResult.TranscodeResult.Segments[segmToCheckIndex].PerceptualHashUrl, err)
			return nil, nil, err
		}
	...
	// return nil, nil, errors.New("bad one: todo")
}
...
func NewSessionManager(node *core.LivepeerNode, params *core.StreamParameters, sel BroadcastSessionsSelector) *BroadcastSessionsManager {
	...
	bsm := &BroadcastSessionsManager{
		...
		sessMap: make(map[string]*BroadcastSession),
		createSessions: func() ([]*BroadcastSession, error) {
			return selectOrchestrator(node, params, numOrchs, sus, common.Score_Trusted)
		},
		sessLock: &sync.Mutex{},
		numOrchs: numOrchs,
...
func selectOrchestrator(n *core.LivepeerNode, params *core.StreamParameters, count int, sus *suspender, score uint) ([]*BroadcastSession, error) {
	if n.OrchestratorPool == nil {
		glog.Info("No orchestrators specified; not transcoding")
		return nil, errDiscovery
	}

	tinfos, err := n.OrchestratorPool.GetOrchestrators(count, sus, params.Capabilities, score)
	if len(tinfos) <= 0 {
	...
	glog.Infof("===> got %d infos %+v", len(tinfos), tinfos)
	...
	glog.Infof("===> selectOrch with score %d returns %d sessions", score, len(sessions))
	return sessions, nil
}
...
func processSegment(cxn *rtmpConnection, seg *stream.HLSSegment) ([]string, error) {
	...
	for len(attempts) < MaxAttempts {
		...
		urls, info, err = transcodeSegment2(cxn, seg, name, sv)
...
func transcodeSegment2(cxn *rtmpConnection, seg *stream.HLSSegment, name string,
	...
	nonce := cxn.nonce
	// cpl := cxn.pl
	sessions, calcPerceptualHash := cxn.sessManager2.selectSessions()
	glog.Infof("===============================================")
	glog.Infof("selected %d sessions, calcPerceptualHash %v", len(sessions), calcPerceptualHash)
	...
	if len(sessions) == 1 {
		...
		cxn.sessManager2.pushSegInFlight(sess, seg)
		res, err := SubmitSegment(sess, seg, nonce, false)
		if err != nil || res == nil {
			if isNonRetryableError(err) {
				cxn.sessManager2.completeSession(sess)
				return nil, info, err
			}
			cxn.sessManager2.suspendAndRemoveOrch(sess)
			if res == nil && err == nil {
		...
		sess, results, err := cxn.sessManager2.chooseResults(resc, submittedCount)
		if err != nil {
			glog.Errorf("Error choosing results: err=%v", err)
			return nil, info, err
		}
		glog.Infof("===> sessions to use %s", sess.OrchestratorInfo.Transcoder)
		for _, usedSession := range sessions {
			if usedSession.OrchestratorInfo.Transcoder != sess.OrchestratorInfo.Transcoder {
				glog.Infof("===> completing session %s", usedSession.OrchestratorInfo.Transcoder)
				cxn.sessManager2.completeSession(usedSession)
				// cxn.sessManager2.completeSession(updateSession(sess, res))
			}
		}
...
func prepareForTranscoding(cxn *rtmpConnection, sess *BroadcastSession, seg *stream.HLSSegment, name string) (*stream.HLSSegment, error) {
...
func verify(verifier *verification.SegmentVerifier, cxn *rtmpConnection,
	...
	if verification.IsRetryable(err) {
		...
		cxn.sessManager2.removeSession(sess)
	}
	if accepted != nil {
